-- =====================================================
-- VTS ADMIN DATABASE SCHEMA (SAFE VERSION)
...
-- =====================================================
-- CHECK AND CREATE COMPANIES TABLE SAFELY
-- =====================================================

-- First, let's check if companies table exists and create it properly
DO $$
BEGIN
    ...
    ELSE
        -- Check if companies table has the required id column
        IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'companies' AND column_name = 'id') THEN
            -- Add id column if it doesn't exist
            ALTER TABLE companies ADD COLUMN id UUID PRIMARY KEY DEFAULT uuid_generate_v4();
            RAISE NOTICE 'Added id column to existing companies table';
        END IF;
...
BEGIN
    RAISE NOTICE '==============================================';
    RAISE NOTICE 'VTS ADMIN TABLES SETUP COMPLETED SUCCESSFULLY';
    RAISE NOTICE '==============================================';
    RAISE NOTICE 'Next: Run 02_admin_functions.sql';
END $$;
