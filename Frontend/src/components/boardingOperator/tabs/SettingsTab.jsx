...
export default function SettingsTab() {
  ...
  const saveProfile = async () => {
    try { await supabase.from('users').update({ email, phone }).eq('user_id', window.userId); alert('Profile updated'); } catch { alert('Failed'); }
  };
  const resetPassword = async () => {
    try { await supabase.auth.updateUser({ password }); setPassword(''); alert('Password updated'); } catch { alert('Failed'); }
  };
  return (
    <Box>
      <DashboardCard title="Personal Profile" variant="outlined">
      ...
      <Box sx={{ mt: 2 }}>
        <DashboardCard title="Password" variant="outlined">
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
            ...
            <Button variant="contained" disabled={!password} onClick={resetPassword}>Reset Password</Button>
          </Box>
        </DashboardCard>
      </Box>
    </Box>
