...
export default function SettingsTab() {
  ...
  const saveProfile = async () => { try { await supabase.from('users').update({ email, phone }).eq('user_id', window.userId); alert('Updated'); } catch { alert('Failed'); } };
  const resetPassword = async () => { try { await supabase.auth.updateUser({ password }); setPassword(''); alert('Password updated'); } catch { alert('Failed'); } };
  return (
    <Box>
